...
- One way to use the `ansible_host`, `domain_name`, `api_domain_name`, `mediawiki_domain_name`, `grafana_domain_name` and `website_analytics_domain_name` variables is to let them as is (`tournesol-vm`, `tournesol-api`, `tournesol-wiki`, `tournesol-grafana` and `tournesol-webanalytics`) and to put a `<VM_IP> tournesol-vm tournesol-api tournesol-wiki tournesol-grafana tournesol-webanalytics` line in your `/etc/hosts` file
