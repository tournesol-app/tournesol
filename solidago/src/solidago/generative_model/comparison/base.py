...
class ComparisonGenerator(StateFunction):
    def main(self, users: Users, entities: Entities, made_public: MadePublic, comparisons: Comparisons) -> Comparisons:
        ...
        for (username, left_name, right_name, criterion), comparisons_list in comparisons:
            filled_comparisons[username, left_name, right_name, criterion] = list()
            for index, comparison in enumerate(comparisons_list):
                ...
                filled_comparisons.add_row((user, left, right, criterion), dict(comparison) | {
