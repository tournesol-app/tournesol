...
class Independent(GeneratorStep):
    ...
    def __call__(self, users: Users, entities: Entities) -> tuple[MadePublic, Ratings, Comparisons]:
        made_public, ratings, comparisons = MadePublic(), Ratings(), Comparisons()
        for user in users:
            evaluated_entities = self.select_entities(user, entities)
            self.user_engagement(user, evaluated_entities, made_public, ratings, comparisons)
        return made_public, ratings, comparisons
    ...
    def user_engagement(self, 
    ...
    ) -> tuple[MadePublic, Ratings, Comparisons]:
        for entity in eval_entities:
            ...
            if self.rate(user, entity, eval_entities):
                ratings[user, "default", entity] = Rating()
