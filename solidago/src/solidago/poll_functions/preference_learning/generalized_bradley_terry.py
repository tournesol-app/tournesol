...
class GeneralizedBradleyTerry(PreferenceLearning):
    ...
    def user_learn(self,  # type: ignore
        ...
        for criterion in comparisons.keys("criterion"):
            ...
            for score in scores:
                if not score.isnan():
                    model.directs.set(score, entity_name=score["entity_name"], criterion=criterion)
        return model
    
    def user_learn_criterion(self, 
        entities: Entities,
        comparisons: Comparisons, # keynames == ["left_name, right_name"]
        init: Scores | None = None, # keynames == ["entity_name"]
    ) -> Scores: # keynames == ["entity_name"]
        """
        Returns
        -------
        out: dict
        ...
        """
        init = Scores(keynames=["entity_name"]) if init is None else init
        ...
        for i, entity in enumerate(entities):
            ...
            if lefts[i] == self.max_uncertainty and rights[i] == self.max_uncertainty:
                continue
            scores.append(score, entity_name=entity.name)
        return scores

    def init_values(self, entities: Entities, init: Scores) -> NDArray:
    ...
    def compute_uncertainties(self,
        ...
        kwargs: dict[str, Any] = dict(f=self.translated_negative_log_likelihood, error=1e-1)
        def solve(**kwargs): 
        ...
        for entity_index, (other_indices, comparison_values, comparison_maxs) in enumerate(entity_comparisons):
            ...
            value_diffs = values[entity_index] - values[other_indices]
