...
class NumbaCoordinateDescentGBT(GeneralizedBradleyTerry):
    ...
    @property
    @abstractmethod
    def cumulant_generating_function_derivative(self) -> Callable[[NDArray[np.float64]], NDArray[np.float64]]: # type: ignore
    ...
    def compute_values(self,
        ...
        return CoordinateDescent(self.convergence_error, self.max_iter).coordinate_descent(
            self.partial_derivative, # type: ignore
    ...
    def get_partial_derivative_args(self, 
        entities: Entities, 
        comparisons: Comparisons, # keynames == ["entity_name", "other_name"]
    ) -> Callable[[int, NDArray], tuple[NDArray[np.float64], NDArray[np.float64]]]:
    ...
    @cached_property
    def partial_derivative(self) -> Callable[[float, NDArray[np.float64], NDArray[np.float64]], float]:
        ...
        def njit_partial_derivative(
            entity_value: float,
            compared_values: NDArray[np.float64], 
            normalized_comparisons: NDArray[np.float64], 
        ) -> float:
            ...
            return float(prior_derivative + nll_derivative)
        
        return njit_partial_derivative

    def gradient(self, values: NDArray[np.float64], entities: Entities, comparisons: Comparisons) -> NDArray[np.float64]:
...
class NumbaUniformGBT(NumbaCoordinateDescentGBT, UniformGBT):
    ...
    def cumulant_generating_function_derivative(self) -> Callable[[NDArray[np.float64]], NDArray[np.float64]]: # type: ignore
        ...
        """
        @njit
        def njit_cumulant_generating_function_derivative(value_diffs: NDArray[np.float64]) -> NDArray[np.float64]:
            return np.where(
