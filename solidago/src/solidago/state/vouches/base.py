...
class Vouches:
    ...
    def __getitem__(self, args: tuple[Union[str, "User"], Union[str, "User"], str]) -> tuple[float, float]:
        ...
        voucher = args[0] if isinstance(args[0], (int, str)) else args[0].name
        vouchee = args[1] if isinstance(args[1], (int, str)) else args[1].name
    ...
    def to_df(self):
        return DataFrame([
            Series({ "kind": kind, "by": voucher, "to": vouchee, "weight": out[0], "priority": out[1] })
