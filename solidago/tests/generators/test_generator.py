def test_svd_user_generator():
    ...
    users = generators.users.AddColumn("trustworthy", Bernoulli(0.8))(users)
...
def test_evaluation_generator():
    ...
    public_settings, empty_ratings, empty_comparisons = engagement_generator(users, entities)
    rating_generator = generators.ratings.Independent(generators.ratings.Noisy(Normal()))
    ratings = rating_generator(users, entities, public_settings, empty_ratings)
    comparison_generator = generators.comparisons.Independent(generators.comparisons.KnaryGBT(21, 10))
    comparisons = comparison_generator(users, entities, public_settings, empty_comparisons)
