...
def test_learned_models():
    scaled_users, scaled = functions.scaling.Mehestan(max_workers=1).fn(users, entities, public_settings, user_models)
    assert len(scaled_users) == len(users)
    assert len(user_models) == len(scaled)

def test_standardize():
    standardized_models = functions.scaling.LipschitzStandardize(lipschitz=1000, max_workers=1).fn(entities, user_models)
