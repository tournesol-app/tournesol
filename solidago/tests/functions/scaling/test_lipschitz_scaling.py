...
def test_standardize():
    scale = functions.scaling.LipschitzStandardize(lipschitz=1000, max_workers=1)
    standardized_models = scale.fn(entities, user_models)
...
def test_quantile_shift():
    scale = functions.scaling.LipschitzQuantileShift(lipschitz=1000, max_workers=1)
    base_models = UserModels(user_directs=user_models.user_directs)
    shifted_models = scale.fn(entities, base_models)
