...
from solidago import functions as pf
...
_modules = [
    ...
    pf.preference_learning.FlexibleGeneralizedBradleyTerry(
        ...
        discard_ratings=False,
    ),
    pf.voting_rights.AffineOvertrust(privacy_penalty=0.5, min_overtrust=2., overtrust_ratio=.1),
    pf.scaling.Mehestan(
    ...
    pf.scaling.LipschitzStandardize(dev_quantile=.9, lipschitz=10., error=1e-2, n_sampled_entities_per_user=100),
    pf.scaling.LipschitzQuantileShift(quantile=.15, lipschitz=10., error=1e-2, target_score=.21, n_sampled_entities_per_user=100),
    pf.aggregation.EntitywiseQrQuantile(quantile=.2, lipschitz=.1, error=1e-5),
    pf.post_process.Squash(score_max=100)
]

class TestPipeline(Sequential):
    def __init__(self, max_workers: int = 1, seed: int | None = None):
        super().__init__(_modules, "TestPipeline", max_workers, seed)
