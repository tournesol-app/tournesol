...
which bounds the maximal impact of a councillor.
More precisely, we consider a slightly generalized setting with councillor volumes.
...
i.e. $v'$ removes councillor $u$'s voting right.
\end{proof}
...
be Lipschitz continuous as a function of councillors' voting rights
(using the $\ell_1$ norm as a metric for voting rights).
In our context, we would have $\bbX \triangleq \setR^E$ (councillor ballots)
...
that if Alice only follows a single councillor,
then her recommendations will be heavily influenced by this single councillor.
