...
export default () => {
  ...
  const saveFormData = (e) => {
    ...
    api.loginSignupResetPasswordPartialUpdate({ patchedResetPassword: data },
      (err, _data) => {
        ...
        if (!err) {
          ...
          setRequested(false);
          setJsonError(null);
          history.push('/email_show_instructions_reset');
        } else {
  ...
  if (!formData && !requested) {
    ...
    getSchema().then((apiSchema) => {
      const apiSchemaTransformed = {
        ...
        ...apiSchema.components.schemas.PatchedResetPassword,
      };
    ...
    <div className={classes.root}>
      {JSONSchema && (
        ...
        >
          {success && (
          <>
            <Alert severity="success" className="class_success_data_save">OK</Alert>
          </>
          )}
          {error && !success &&
          <Alert severity="error" className="class_error_data_save">Error: {error}</Alert>}

          <Button color="primary" variant="contained" type="submit" id="id_reset_submit">
            Reset password
          </Button>
        </Form>
      )}
