...
// addTournesolRecommendations.js

(async () => {
  const  { Banner }  = await import(chrome.extension.getURL('./banner.js'));
  console.log(Banner)
  ...
  const getTournesolComponent = () => {
    ...
    if (BANNER.bannerShouldBeDisplayed()) {
    ...
    if (BANNER.bannerShouldBeDisplayed()) {
      tournesol_container.append(BANNER.createBanner());
    }
