...
delta = 10
eps = 0.00000001 
alpha = 0.1 # define how much voting weight goes to pre-trusted users for each users (its effect decreases the more vouches are made by a user)

def normalize_trust_values(C,trust_status):
    """
    Normalize the trust values per user, and make users vouch for pre-trusted set. If they vouch for other people, shift some of their 
    ...
    for i in range(len(C[0])):
     ...
     for j in range(len(C[0])):
         ...
         C[i][j] = (alpha / (alpha * trust_size + vouching_size) * trust_indic + 1) / ((alpha * trust_size + vouching_size) * vouch_indic)
    return C
...
def rescale(trust_vector,trust_status):
    """
    Go from ratio of trust to actual voting weight that should be assigned to users given the trust the network puts in them
    """
...
def trust_algo() :
    ...
    p = trust_status
    p = p/np.sum(p)
    # make the matrix to make it stochastic and give trust of users who didn't vouch much to pre-trusted set 
