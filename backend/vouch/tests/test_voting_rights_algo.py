...
def test_everyone_trusted():
    ...
    np.testing.assert_array_equal(compute_voting_rights(trust_scores, privacy_penalties), [1, 1, 1, 1])


def test_everyone_trusted_some_penalized():
    trust_scores, privacy_penalties = np.array([1, 1, 1, 1]), np.array([0.5, 0.5, 1, 1])
    np.testing.assert_array_equal(compute_voting_rights(trust_scores, privacy_penalties), [0.5, 0.5, 1, 1])


def test_untrusted_less_than_bias_get_full_voting_right():
    np.testing.assert_array_equal(
        compute_voting_rights(np.array([0, 0.5, 0.5, 1, 1]), np.ones(shape=5)), [1, 1, 1, 1, 1]
    )


def test_untrusted_and_penalized_less_than_bias_get_penalized_voting_right():
    np.testing.assert_array_equal(
        compute_voting_rights(np.array([0, 0.5, 0.5, 1, 1]), np.array([0.7, 0.7, 0.7, 1, 1])), [0.7, 0.7, 0.7, 1, 1]
    )
...
def test_untrusted_get_partial_voting_right(n_trusted, n_non_trusted):
    ...
    np.testing.assert_array_equal(
        compute_voting_rights(np.array([0] * n_non_trusted + [1] * n_trusted), np.ones(shape=n_non_trusted+n_trusted)),
...
def test_min_voting_right_more_than_min_trust(n_random_users):
    trust_scores = np.random.random(size=(n_random_users,))
    min_voting_right = compute_voting_rights(trust_scores, np.ones(shape=n_random_users)).min()
