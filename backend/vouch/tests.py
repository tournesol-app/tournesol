...
# # ======= unit tests - vouching connections =======        
        ...
        vouching = np.array((len(user_set),len(user_set)))
        for i in range(len(user_set)):
            for j in range(len(user_set)):
                if i != j :
                    if (random.randint(0,10) < 10):
                        vouching[i][j] = vouch.objects.create(vouching = user_set[i],vouchedfor=user_set[j],trust_value = random.randint(0,10)/10.0)
