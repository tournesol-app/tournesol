...
class TestMlTrain(TestCase):
    def setUp(self) -> None:
        ...
        EmailDomain.objects.create(
            domain="@not_verified.test", status=EmailDomain.STATUS_REJECTED
        )
        ...
        for i in range(10):
            not_trusted_user = UserFactory(
                email=f"not_trusted_user{i}@not_verified.test"
            )
            ComparisonCriteriaScoreFactory(
    ...
    def test_ml_train_skip_untrusted(self):
        ...
        contributor_rating_user_1 = ContributorRating.objects.get(
            user=self.user1, entity=self.video1
        )
        contributor_rating_user_2 = ContributorRating.objects.get(
            user=self.user2, entity=self.video1
        )

        self.assertLess(
            EntityCriteriaScore.objects.get(entity_id=self.video1.id).score, 0
        )
        self.assertGreater(
            EntityCriteriaScore.objects.get(entity_id=self.video2.id).score, 0
        )
    ...
    def test_ml_train_on_trusted_and_all_users(self):
        ...
        contributor_rating_score_user_1 = ContributorRatingCriteriaScore.objects.get(
            contributor_rating=contributor_rating_user_1
        ).score
        contributor_rating_score_user_2 = ContributorRatingCriteriaScore.objects.get(
            contributor_rating=contributor_rating_user_2
        ).score
        ...
        self.assertEqual(
            ...
            contributor_rating_score_user_1,
        )
        self.assertEqual(
            ContributorRatingCriteriaScore.objects.get(
                contributor_rating=contributor_rating_user_2
            ).score,
            contributor_rating_score_user_2,
        )
