...
class ExportTest(TestCase):
    def setUp(self) -> None:
        ...
        self.comparison = ComparisonFactory(user=self.user_with_comparisons, entity_1=self.video1, entity_2=self.video2)
        ...
        ContributorRating.objects.create(entity=self.video_public_1, user=self.public_comparisons, is_public=True)
        ContributorRating.objects.create(entity=self.video_public_2, user=self.public_comparisons, is_public=True)
        ContributorRating.objects.create(entity=self.video_private_3, user=self.public_comparisons, is_public=False)
        self.comparison_public = ComparisonFactory(user=self.public_comparisons, entity_1=self.video_public_1, entity_2=self.video_public_2)
        ComparisonCriteriaScoreFactory(comparison=self.comparison_public, score=5, criteria="largely_recommended")
        self.comparison_private = ComparisonFactory(user=self.public_comparisons, entity_1=self.video_public_1, entity_2=self.video_private_3)
    ...
    def test_not_authenticated_can_download_public_comparisons_multiple_users(self):
        ...
        ContributorRating.objects.create(entity=self.video_public_3, user=self.public_comparisons2, is_public=True)
        ContributorRating.objects.create(entity=self.video_public_4, user=self.public_comparisons2, is_public=True)
        self.comparison_public2 = ComparisonFactory(user=self.public_comparisons2, entity_1=self.video_public_3, entity_2=self.video_public_4)
