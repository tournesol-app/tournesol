...
class Node:
    def __init__(
            ...
            mask, t_par, s_par, model, stable, tot_comps,
        ...
        stable (bool): True if the model has converged, False to get trained
        tot_comps (int): number of comparisons that user had for last training
        ...
        self.s_par = s_par
        self.model = model
        self.stable = stable and len(vid1) == tot_comps
        ...
        self.nb_loc_vids = torch.sum(mask.int())  # nb of videos rated by user
        factor = self.nb_loc_vids / (300 + self.nb_loc_vids)
